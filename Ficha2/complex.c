...
complex* complex_div(complex* z, complex* w){
  complex* num = complex_mul(z, complex_conj(w));
  double denom = complex_re(complex_mul(w, complex_conj(w)));
  return complex_new(num -> x/denom, num -> y/denom);
}

complex* complex_conj(complex* z){
  return complex_new(z->x, z->y*-1);
}
